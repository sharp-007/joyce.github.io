(() => {
  ...
  function loadAIChat() {
    ...
    if (!config.enabled || !config.app_token) {
      placeholder.innerHTML = `
        <div class="ai-chat-error">
          ...
          <p style="font-size: 0.78rem; color: #94a3b8; margin-top: 8px;" data-en="Please contact Joyce to enable this feature" data-zh="请联系 Joyce 开启此功能">请联系 Joyce 开启此功能</p>
        </div>
      `;
      reapplyLang();
      return;
    ...
    const baseUrl = config.base_url || 'https://udify.app';
    const token = config.app_token;
    const chatbotType = config.chatbot_type || 'chatbot';
    
    const iframe = document.createElement('iframe');
    iframe.src = `${baseUrl}/${chatbotType}/${token}`;
    ...
    iframe.onerror = function() {
      placeholder.innerHTML = `
        <div class="ai-chat-error">
          ...
          <a href="mailto:joyce@example.com" data-en="Contact Joyce" data-zh="联系 Joyce">联系 Joyce</a>
        </div>
      `;
      reapplyLang();
    };
