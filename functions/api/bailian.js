...
//
// 关键设计：用 upstream.body.pipeThrough(new TransformStream({ transform }))。
// 这是 EdgeOne Pages Functions / Cloudflare Workers 上最稳定的 SSE 代理写法，
// 因为运行时会自动管理整条流的生命周期 —— 不会像 IIFE 那样在 onRequest 同步
// 返回 Response 之后被销毁（IIFE 写法会出现 ERR_CONNECTION_CLOSED）。
//
// 每条事件附带服务端时间戳 ts（ms 相对首字节）方便前端 DevTools 排查链路延迟。
function streamBailianSSE(upstream, origin) {
  ...
  const transformer = new TransformStream({
    start(controller) {
      // 立即发送 2KB 的 SSE 注释 padding，强制中间代理 / 浏览器立刻打开通道
      controller.enqueue(encoder.encode(':' + ' '.repeat(2048) + '\n\n'));
    },
    transform(chunk, controller) {
      // 第一次拿到上游数据 → 在控制台一次性记录首字节耗时（仅本地调试用）
      if (!firstChunkSent) {
        firstChunkSent = true;
      }
      ...
      for (const ev of events) {
        ...
        // 本身就是当次 delta 片段（参考百炼官方文档 Python 示例 5694-5698 行）
  ...
  const responseStream = upstream.body.pipeThrough(transformer);
